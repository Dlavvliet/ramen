...
export default function OrderPage({ cart, onClose, onOrderSuccess }) {
    ...
    try {
      ...
      const response = await fetch('https://ramen-production-4a74.up.railway.app/orders', {
        ...
        headers: {
          ...
          'Authorization': `Bearer ${getToken()}`   // ← Добавлено
        },
      ...
      alert(err.message || 'Ошибка при оформлении заказа. Попробуйте еще раз.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleCloseSuccess = () => {
    onOrderSuccess(); // очищаем корзину и закрываем модальное окно
  };
